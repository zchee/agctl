//! `agentctl completions` — a shell completion script generated straight
...
/// Writes a shell completion script for `agentctl` to `out`.
///
...
/// script (`agentctl completions zsh | head -1`). Buffering first means the

...
pub mod swap_exit {
    ...
    ///
    /// The live session logged in as somebody else since that swap, and the
    /// reversal would file that account's credential as the installed one's.
    /// Decided in Phase A right after the item read, before any lock. `--json`
    /// gives it `reason: "live_undo_foreign_login"` and no `refusal` member.
    pub const LIVE_UNDO_ITEM_CHANGED: i32 = 27;
    /// Nothing can say whose credential the live item holds (S24).
    ///
    /// The profile GET did not answer — `reason: "profile_unavailable"` — or the
    /// item's access token has expired and no write of agctl's own put those
    /// bytes there — `reason: "live_token_expired"`. Decided in Phase A right
    /// after the item read, before every lock, the audit log, the prompt and
    /// the POST, so nothing is written. No `refusal` member.
    ///
    /// A fresh number rather than one of 25, 26 or 28, which S23b's temporary
    /// refusals used and S24 retired: a script that learned those must not see
    /// them change meaning.

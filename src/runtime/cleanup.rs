...
// Only `emergency` has a caller so far; the registration side is used by the
// credential writer and the TUI, which land in W1 and W3. Scoped to the
// non-test build because the tests below exercise all of it, and spelled
// `expect` so it starts warning once it is no longer true.
#![cfg_attr(not(test), expect(dead_code, reason = "registration side lands with W1 and W3"))]
